...
public class XmlSchemaEnhancer {
	...
	public static <T,C> void addFacets(ValuePropertyInfo<T,C> vp, SimpleRestriction sr) {
                if(!hasFacets(vp))
                        return;
	...
	protected static <T extends Annotation> T getAnnotationOfProperty(
			Class<?> parent, String fieldName, Class<T> annoClass) throws Exception {
		try {
			Field field = null;
	...
	private static <T extends Annotation> T convertToAnnotation(
		...
		try {
			...
			T anno = (T) Proxy.newProxyInstance(
				expectedClass.getClassLoader(),
				new Class[] { expectedClass },
						...
						try {
							...
							if(o != null) {
								...
								if(!componentClass.isPrimitive() && 
									...
									ClassLoader cl = expectedClass.getClassLoader() != null ?
											expectedClass.getClassLoader() : ClassLoader.getSystemClassLoader();
									String name = componentClass.getName();
									if(cl.loadClass(name) != componentClass) {
										/* we need to import/convert the class into 
										 * the classloader of expectedClass */
	...
	private static <T,C> TypedXmlWriter getRestriction(
			AttributePropertyInfo<T,C> info, TypedXmlWriter obj, TypedXmlWriter w) {
		if(w == null) {
