...
public class XmlSchemaEnhancer {
    ...
    public static <T, C> void addFacets(TypeRef<T, C> t, LocalElement e) {
        ...
        for (String facetName : facets.keySet()) {
            ...
            for (String facetValue : facets.get(facetName)) {
                ...
                restriction._element(new QName(NS_XSD, facetName), TypedXmlWriter.class)
    ...
    public static <T, C> void addFacets(Facets facetsAnno, TypedXmlWriter restriction) {
        // TODO to reduce code duplication, implement and use this
        // method instead of the other addFacets methods!
    }
    ...
    public static <T, C> void addXsdAnnotations(javax.xml.bind.annotation.Annotation anno, TypedXmlWriter obj) {
        ...
        for (Documentation doc : anno.documentation()) {
            TypedXmlWriter w = annoEl._element(new QName(NS_XSD, "documentation"), TypedXmlWriter.class);
    ...
    public static <T, C> boolean hasXsdAnnotations(AttributePropertyInfo<T, C> ap) {
        javax.xml.bind.annotation.Annotation anno = getXsdAnnotationAnnotation(ap);
        return anno != null;
    }

    public static <T, C> boolean writeCustomOccurs(TypeRef<T, C> t, LocalElement e, boolean isOptional,
                                                   boolean repeated) {

        MaxOccurs max = null;
        MinOccurs min = null;
        try {
            max = (MaxOccurs)getAnnotationOfProperty(t.getSource(), MaxOccurs.class);
        } catch (Exception e2) {
            logger.log(Level.WARNING, "Unable to get @MaxOccurs annotation from type " + t, e2);
        }
        try {
            min = (MinOccurs)getAnnotationOfProperty(t.getSource(), MinOccurs.class);
        } catch (Exception e2) {
    ...
    private static <T, C> javax.xml.bind.annotation.Annotation getXsdAnnotationAnnotation(PropertyInfo<T, C> t) {
        ...
        try {
            Object value = getAnnotationOfProperty(t, javax.xml.bind.annotation.Annotation.class);
            if (value instanceof javax.xml.bind.annotation.Annotation) {
                anno = (javax.xml.bind.annotation.Annotation)value;
            }
        } catch (Exception e2) {
            logger.log(Level.WARNING, "Unable to get XSD Annotation annotation from type " + t, e2);
        }
        ...
        try {
            ...
            logger.log(Level.WARNING, "Unable to get XSD AppInfo annotation from type " + t, e2);
        }
        ...
        try {
            ...
            logger.log(Level.WARNING, "Unable to get XSD Documentation annotation from type " + t, e2);
        }

        return getXsdAnnotationAnnotation(anno, doc, appinfo);
    }

    protected static <T, C> javax.xml.bind.annotation.Annotation getXsdAnnotationAnnotation(javax.xml.bind.annotation.Annotation _anno,
                                                                                            Documentation _doc,
                                                                                            AppInfo _appinfo) {
        javax.xml.bind.annotation.Annotation anno = null;
        ClassLoader cl = _anno != null ? _anno.getClass().getClassLoader() : _doc != null ? _doc.getClass()
            .getClassLoader() : _appinfo != null ? _appinfo.getClass().getClassLoader() : ClassLoader
        ...
        anno = (javax.xml.bind.annotation.Annotation)Proxy.newProxyInstance(cl, new Class<?>[] {
    ...
    private static <T, C> Object getAnnotationOfProperty(PropertyInfo<T, C> info,
        ...
        if (!(info.parent().getType() instanceof Class<?>))
            return null;
        return getAnnotationOfProperty((Class<?>)info.parent().getType(), info.getName(), annoClass);
    }

    protected static <T extends Annotation> T getAnnotationOfProperty(Class<?> parent, String fieldName,
                                                                      Class<T> annoClass) throws Exception {
        try {
            Field field = null;
            for (Field f : parent.getDeclaredFields()) {
                if (f.getName().equals(fieldName)) {
                    ...
                    if (getAnnotation(f, XmlElement.class) != null) {
                        XmlElement e = (XmlElement)getAnnotation(f, XmlElement.class);
                    ...
                    if (getAnnotation(f, XmlAttribute.class) != null) {
                        XmlAttribute a = (XmlAttribute)getAnnotation(f, XmlAttribute.class);
            ...
            throw new RuntimeException("Could not get annotation '" + annoClass.getSimpleName()
                                       + "' of field " + fieldName + " of class " + parent, e);
        }
    }

    protected static <T extends Annotation> T getAnnotation(Field field, Class<T> annoClass) {
        for (Annotation anno : field.getAnnotations()) {
            ...
            // }
            // }
            if (annoClass.equals(anno.getClass()) || annoClass.isAssignableFrom(anno.getClass())) {
                return (T)anno;
            }
        }
        return null;
    ...
    private static <T extends Annotation> T convertToAnnotation(final InvocationHandler handler,
        ...
        try {
            ...
            if (!expectedClass.getName().equals(type.getName())) {
                throw new RuntimeException("Not the expected annotation type: " + type + " != "
                                           + expectedClass);
            }
            T anno = (T)Proxy.newProxyInstance(expectedClass.getClassLoader(), new Class[] {
                expectedClass
                    ...
                    try {
                        ...
                        if (o != null) {
                            ...
                            if (!componentClass.isPrimitive()
                                && !componentClass.getName().startsWith("java.lang")) {
                                ClassLoader cl = expectedClass.getClassLoader() != null ? expectedClass
                                    .getClassLoader() : ClassLoader.getSystemClassLoader();
                                String name = componentClass.getName();
                                if (cl.loadClass(name) != componentClass) {
                                    /*
                                     * we need to import/convert the class into the classloader of
                                     * expectedClass
                                     */
                                    if (componentClass.getName().endsWith("WhiteSpace")) {
                                        o = WhiteSpace.valueOf(o.toString());
                                    } else {
    ...
    private static <T, C> TypedXmlWriter getRestriction(TypeRef<T, C> t, TypedXmlWriter obj, TypedXmlWriter w) {
        if (w == null) {
            TypedXmlWriter st = obj._element(new QName(NS_XSD, "simpleType"), TypedXmlWriter.class);
            TypedXmlWriter r = st._element(new QName(NS_XSD, "restriction"), TypedXmlWriter.class);
            QName schemaType = t.getSource() == null ? null : t.getSource().getSchemaType();
            if (schemaType == null)
    ...
    private static <T, C> TypedXmlWriter getXsdAnnotation(TypedXmlWriter obj, String annoID,
    ...
    protected static Map<String, List<String>> getDefinedFacets(Facets facetsAnnotation) throws Exception {
        ...
        for (Method m : annoMethods) {
            ...
            FacetDefinition facetDefinition = m.getAnnotation(FacetDefinition.class);
